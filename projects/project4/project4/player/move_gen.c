...
// King orientations
char* king_ori_to_rep[2][NUM_ORI] = {{"NN", "EE", "SS", "WW"}, {"nn", "ee", "ss", "ww"}};

// Pawn orientations
char* pawn_ori_to_rep[2][NUM_ORI] = {{"NW", "NE", "SE", "SW"}, {"nw", "ne", "se", "sw"}};
...
move_t move_of(ptype_t typ, rot_t rot, square_t from_sq, square_t int_sq, square_t to_sq) {
    return ((typ & PTYPE_MV_MASK) << PTYPE_MV_SHIFT) | ((rot & ROT_MASK) << ROT_SHIFT) |
           ((from_sq & FROM_MASK) << FROM_SHIFT) |
           ((int_sq & INTERMEDIATE_MASK) << INTERMEDIATE_SHIFT) | ((to_sq & TO_MASK) << TO_SHIFT);
}
                ...
                case KING:
                    ...
                    for (int d = 0; d < 8; d++) {
                        ...
                        if (!is_swap) {
                            ...
                            WHEN_DEBUG_VERBOSE({
                                move_to_str(move_of(typ, (rot_t)0, sq, (square_t)sq, dest), buf,
                                            MAX_CHARS_IN_MOVE);
                        ...
                        if (is_swap) {
                            for (int d = 0; d < 8; d++) {
                                ...
                                WHEN_DEBUG_VERBOSE(char buf[MAX_CHARS_IN_MOVE]);
                                WHEN_DEBUG_VERBOSE({
                                    move_to_str(move_of(typ, (rot_t)0, sq, dest, final_dest), buf,
                                                MAX_CHARS_IN_MOVE);
                                    DEBUG_LOG(1, "Before: %s ", buf);
                                });
                                tbassert(move_count < MAX_NUM_MOVES, "move_count: %d\n",
                                ...
                                sortable_move_list[move_count++] =
                                    move_of(typ, (rot_t)0, sq, dest, final_dest);
                            ...
                            for (int rot = 1; rot < 4; ++rot) {
                                tbassert(move_count < MAX_NUM_MOVES, "move_count: %d\n",
                                         move_count);
                                sortable_move_list[move_count++] =
                                    move_of(typ, (rot_t)rot, sq, dest, dest);
                            }
                        }
                    }
...
int generate_all_with_color(position_t* p, sortable_move_t* sortable_move_list, color_t color) {
    color_t color_to_move = color_to_move_of(p);
...
square_t fire_laser(position_t* p, color_t c) {
    ...
    tbassert(ptype_of(p->board[sq]) == KING, "ptype: %d\n", ptype_of(p->board[sq]));
...
void low_level_make_move(position_t* old, position_t* p, move_t mv) {
    ...
    tbassert(old->key == compute_zob_key(old), "old->key: %" PRIu64 ", zob-key: %" PRIu64 "\n",
    ...
    *p = *old;
    ...
    tbassert(p->board[to_sq] < (1 << PIECE_SIZE) && p->board[to_sq] >= 0, "p->board[to_sq]: %d\n",
             p->board[to_sq]);
    ...
    if (to_sq != from_sq) {  // move, not rotation
        ...
        if (int_sq != from_sq) {
            ...
            if (int_sq != to_sq) {
                ...
                p->board[int_sq] =
                    to_piece;  // Pieces should move the order from->to, to->int, int->from
        ...
        if (ptype_of(int_piece) == KING) {
            if (is_double_move) {
                p->kloc[color_of(int_piece)] = from_sq;
            }
        }
    ...
    tbassert(p->key == compute_zob_key(p), "p->key: %" PRIu64 ", zob-key: %" PRIu64 "\n", p->key,
             compute_zob_key(p));
...
victims_t make_move(position_t* old, position_t* p, move_t mv) {
    ...
    if (USE_KO) {  // Ko rule
        if (p->key == (old->key ^ zob_color)) {
            ...
            for (fil_t f = 0; f < BOARD_WIDTH; f++) {
                for (rnk_t r = 0; r < BOARD_WIDTH; r++) {
                    if (p->board[square_of(f, r)] != old->board[square_of(f, r)]) {
                        match = false;
                    }
                }
            }
        ...
        if (p->key == old->history->key) {
            ...
            for (fil_t f = 0; f < BOARD_WIDTH; f++) {
                for (rnk_t r = 0; r < BOARD_WIDTH; r++) {
                    if (p->board[square_of(f, r)] != old->history->board[square_of(f, r)]) {
                        match = false;
                    }
                }
            }
...
static uint64_t perft_search(position_t* p, int depth, int ply) {
    ...
    for (i = 0; i < num_moves; i++) {
        ...
        if ((victim_sq = fire_laser(&np, color_to_move_of(p)))) {  // hit a piece
            ...
            tbassert((ptype_of(victim_piece) != EMPTY) && (ptype_of(victim_piece) != INVALID),
        ...
        if (np.victims.zapped_count > 0 && ptype_of(np.victims.zapped) == KING) {
...
void display(position_t* p) {
    ...
    for (rnk_t r = BOARD_WIDTH - 1; r >= 0; --r) {
        ...
        for (fil_t f = 0; f < BOARD_WIDTH; ++f) {
            ...
            tbassert(ptype_of(p->board[sq]) != INVALID, "ptype_of(p->board[sq]): %d\n",
                     ptype_of(p->board[sq]));
            /*if (p->blocked[sq]) {
        printf(" xx");
        continue;
