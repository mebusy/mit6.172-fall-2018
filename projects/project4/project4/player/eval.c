...
int PCENTRAL;  // bonus for Pawns near the center of the board
int PBETWEEN;  // bonus for Pawns between the Kings
// int PCENTRAL;
int KFACE;        // bonus for your King facing the enemy king
int KAGGRESSIVE;  // bonus for the King with more space behind it(to the end of the board)
int MOBILITY;     // how many spaces around your King are free.
int LCOVERAGE;    // measures how much the board near the enemy king is covered by the laser after
                  // making all possible moves from a position
...
ev_score_t pbetween(position_t* p, fil_t f, rnk_t r) {
    bool is_between = between(f, fil_of(p->kloc[WHITE]), fil_of(p->kloc[BLACK])) &&
    ...
    for (fil_t f = 0; f < BOARD_WIDTH; ++f) {
        for (rnk_t r = 0; r < BOARD_WIDTH; ++r) {
            if (coverage_map[square_of(f, r)] < FLT_MAX) {
                ...
                tbassert(manhattan_dist(king_sq, square_of(f, r)) <= coverage_map[square_of(f, r)],
                         "f: %d, r: %d, dist = %d, map: %f\n", f, r,
                         manhattan_dist(king_sq, square_of(f, r)), coverage_map[square_of(f, r)]);

                // printf("before: %f, dist: %d\n", coverage_map[square_of(f, r)], manhattan_dist(king_sq, square_of(f, r)));
                coverage_map[square_of(f, r)] =
                    ((manhattan_dist(king_sq, square_of(f, r))) / (coverage_map[square_of(f, r)]));

                tbassert(mult_dist(square_of(f, r), opp_king_sq) > 0,
...
int mobility(position_t* p, color_t color) {
    ...
    int mobility = 0;
    square_t king_sq = p->kloc[color];
    tbassert(ptype_of(p->board[king_sq]) == KING, "ptype: %d\n", ptype_of(p->board[king_sq]));
    tbassert(color_of(p->board[king_sq]) == color, "color: %d\n", color_of(p->board[king_sq]));
...
// Static evaluation.  Returns score, higher is better
