...
static void set_sort_key(sortable_move_t* mv, sort_key_t key) {
    ...
    *mv = ((((uint64_t)key) & SORT_MASK) << SORT_SHIFT) | (*mv & ~(SORT_MASK << SORT_SHIFT));
    return;
}
...
moveEvaluationResult evaluateMove(searchNode* node, move_t mv, move_t killer_a, move_t killer_b,
    ...
    if (type == SEARCH_SCOUT) {
        result.score = -scout_search(&(result.next_node), search_depth, node_count_serial);
    } else {
        if (node->legal_move_count == 0 || node->quiescence) {
            ...
            if (result.score > node->alpha) {
                result.score =
                    -searchPV(&(result.next_node), node->depth + ext - 1, node_count_serial);
            }
        }
    }
...
bool search_process_score(searchNode* node, move_t mv, int mv_index, moveEvaluationResult* result,
                          searchType_t type) {
    if (result->score > node->best_score) {
        ...
        memcpy(node->subpv + 1, result->next_node.subpv, sizeof(move_t) * (MAX_PLY_IN_SEARCH - 1));
